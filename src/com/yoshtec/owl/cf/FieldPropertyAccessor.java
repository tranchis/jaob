...
public class FieldPropertyAccessor implements PropertyAccessor{
    ...
    /** List of data types */
    private Set<URI> dtUri = new HashSet<URI>();
    ...
    public FieldPropertyAccessor(Field field, URI propUri, XsdTypeMapper typeMapper) {
        this.init(field, propUri, null, typeMapper);
    }

    public FieldPropertyAccessor(Field field, URI propUri, XmlAdapter<?, ?> adapter, XsdTypeMapper typeMapper){
        this.init(field, propUri, adapter, typeMapper);
    }
    
    private void init(Field field, URI propUri, XmlAdapter<?, ?> adapter, XsdTypeMapper typeMapper){
    ...
    private URI reflectOwlType(Type type){
        ...
        if(actualType instanceof Class<?>){
            ...
            if(at.isAnnotationPresent(OwlClass.class)){
                ...
                if(oc.uri().equals(Const.DEFAULT_ANNOTATION_STRING)){
                    ...
                    returi = URI.create("#" + at.getSimpleName());
                } else {
                    returi = URI.create(oc.uri());
                }
